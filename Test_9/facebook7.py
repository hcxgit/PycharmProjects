...
#  相似度矩阵
def similarity_matix(edges, longth_edges, DG, s):
    # 相似度矩阵   第0行和第0列表示类的代号
    ...
    for x in edges:
        j = 1
        for y in edges:
            # s2=similarity2(str(x[0]), str(x[1]), str(y[0]), str(y[1]), s)
            s2=0
            # 同边
            if x == y:
                s_matix[i][j] = 1
            # 有共同点
            elif x[0] == y[0]:  # 取x[1]，y[1]分别在s中的相应位置的值
                s_matix[i][j] = similarity(x[0], x[1], y[1], DG) #+ s2
            elif x[0] == y[1]:
                s_matix[i][j] = similarity(x[0], x[1], y[0], DG) #+ s2
            elif x[1] == y[0]:
                s_matix[i][j] = similarity(x[1], x[0], y[1], DG) #+ s2
            elif x[1] == y[1]:
                s_matix[i][j] = similarity(x[1], x[0], y[0], DG) #+ s2
            # 没有共同点
            else:
                s_matix[i][j] = 0#+s2
            j += 1
...
def clustering(s_matix, edges, max_density):
    ...
    if max_density == 0:  # 标志位   传进来的max_density 为0，则聚类为一个  否则按照max_density聚类
        while len(clusters) > 1:  # 判断聚类是否完成
            ...
            # c.collect()
            gc.collect()
        del clusters,density_list,s_matix
        return ave_density_list
    else:
        max_similar = 1
        while max_similar >= max_density:  # 判断聚类是否完成
            ...
            num_clusters += 1
            gc.collect()
        del s_matix,density_list,ave_density_list,num_clusters
        return clusters


def ego_graph():
    #  获取边列表                                                   #twitter_combined.txt    facebook_combined.txt
...
#         return r_DG, r_l
#  节点属性
def feat():                                         # twitter   facebook
    with open(r'C:\Users\hcx812591452\Desktop\数据集\twitter\262310943.feat') as file_edge:  ##262310943   532617990
        ...
        for l in lines:
            ...
            l = l.split()
            s[index1]=l
    with open(r'C:\Users\hcx812591452\Desktop\数据集\twitter\262310943.egofeat') as f:
        lines2 = f.readlines()
        s[len(lines)][0]=str(262310943)    ####  注意
...
if __name__ == "__main__":
    ...
    s = feat()
    # s=0
    s_matix = np.array(similarity_matix(DG.edges, DG.number_of_edges(),DG,s))
    print(s_matix)
    # ++++++++++++++++++++++++++++++++++
    print(DG.edges)
    ave_density_list = clustering(s_matix, DG.edges, 0)  # 聚为一个类，得到最大分区密度
    max_density = max(ave_density_list)
    print(max_density)
    clusters2 = clustering(s_matix, DG.edges(),max_density)  # 根据最大分区密度聚类
    print(clusters2)
